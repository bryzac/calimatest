...
const comentarios = ((commentArray, comment, id, logged, rol, userLogged, isOwner) => {
        if (commentArray.includes(comment.id)) {
            ...
            if (rol === 'Admin' || userLogged === commentUser || isOwner === 'true') {
                menuComment = `
                    <button id="menuComment-${commentId}" class="absolute text-xs hover:bg-[#f97020ec] flex self-end rounded-2xl">
            ...
            if (logged) {
                toAsk = `
                ${menuComment}
                <button id="ask-btn-${commentId}" class="text-gray-50 text-opacity-90 text-xs text-left flex self-end underline hover:text-white">Responder</button>
                `;
            };
